...
var addAssetTemplate = document.getElementById("tpl-add-asset-dialog")
    .innerHTML;
...
var addAssetFormEl,
    ...
    addAssetTitle,
    artistSelect,
    artistSelectHelperText,
    artistSelectInput,
    artistSelectMenuEl,
    cancelArtistHelperText,
    cancelProjectHelperText,
    creatorField,
    currentPreviewEl,
    formatRadios,
    linkField,
...
function initialization() {
    ...
    addAssetTitle = new MDCTextField(
        document.querySelector(".add-asset-title-input")
    );

    formatRadios = [].map.call(document.querySelectorAll(".mdc-radio"), function(
        el
    ) {
    ...
    creatorField = new MDCTextField(
        document.querySelector(".add-asset-creator-input")
    );
    ...
    artistSelect = new MDCSelect(
        document.querySelector(".add-asset__category--artist")
    );
    artistSelectHelperText = new MDCSelectHelperText(
        document.querySelector(".add-asset__new-artist--helper-text")
    );
    artistSelectInput = document.getElementById(
        "add-asset__category--artist-input"
    );
    artistSelectMenuEl = document.querySelector(
        ".add-asset__category--artist-select"
    );

    cancelArtistHelperText = new MDCTextFieldHelperText(
        document.querySelector(".add-asset__cancel-artist--helper-text")
    );
    cancelProjectHelperText = new MDCTextFieldHelperText(
        document.querySelector(".add-asset__cancel-project--helper-text")
    );

    newArtistField = new MDCTextField(
        document.querySelector(".add-asset__new-artist--field")
    );
    newArtistForm = document.getElementById("add-asset__new-artist--container");

    newProjectField = new MDCTextField(
        document.querySelector(".add-asset__new-project--field")
    );
    newProjectForm = document.getElementById("add-asset__new-project--container");
    newProjectType = new MDCSelect(
        document.querySelector(".add-asset__new-project--type")
    );
    newProjectTypeInput = document.querySelector("#new-project-type");
    newProjectYear = new MDCSelect(
        document.querySelector(".add-asset__new-project--year")
    );
    newProjectYearInput = document.querySelector("#new-project-year");
    projectSelect = new MDCSelect(
        document.querySelector(".add-asset__category--project")
    );
    projectInputEl = document.querySelector("#cat_project_id");
    projectSelectHelperText = new MDCSelectHelperText(
        document.querySelector(".add-asset__new-project--helper-text")
    );
...
var addAssetDialog = new MDCDialog(
...
addAssetDialog.listen("MDCDialog:opening", () => {
    ...
    wasEscaped
        ...
        addAssetFormEl.addEventListener("change", e => {
            ...
            if (addAssetFormEl.elements.format.value === "format_link") {
                ...
                previewContainerEl.querySelector(
                    "#upload_button_helper-text"
                ).style.display = "none";
    ...
    wasEscaped
        ?
        null :
        addAssetTitle.listen("input", e => {
            checkSaveButton();
        });
    wasEscaped
        ?
        null :
        linkField.listen("input", e => {
            checkSaveButton();
        });
    wasEscaped
        ?
        null :
        newArtistField.listen("input", e => {
            checkSaveButton();
        });
    wasEscaped
        ?
        null :
        newProjectField.listen("input", e => {
            if (e.rangeOffset === 0) {
    ...
    artistSelect.menu_.listen("MDCMenuSurface:opened", () => {
        ...
        artistSelect.menu_.menuSurface_.foundation_.adapter_.setMaxHeight(
            sidebarHeight + "px"
        );
    });
    wasEscaped
        ...
        artistSelect.listen("MDCSelect:change", e => {
            ...
            for (var item in projectsMenuItems) {
                ...
                var projectParentID = parseInt(
                    projectsMenuItems[itemIndex].attributes.parent.value
                );
    ...
    wasEscaped
        ?
        null :
        newProjectType.listen("MDCSelect:change", e => {
    ...
    wasEscaped
        ?
        null :
        newProjectYear.listen("MDCSelect:change", e => {
    ...
    wasEscaped
        ?
        null :
        uploadEditButton.addEventListener("click", e => {
            e.preventDefault();
            if (mediaUploader) {
                mediaUploader.open();
                return;
    ...
    mediaUploader.on("select", function() {
        ...
        switch (attachment.type) {
            ...
            case "audio":
                ...
                previewThumbEl
                    .querySelector(".preview-file-name")
                    .append(attachment.filename);
                previewAudioEl.classList.add(
                    "add-asset__preview-icon",
                    "material-icons"
                );
            ...
            case "application":
                switch (attachment.subtype) {
                    ...
                    case "pdf":
                        ...
                        previewDocEl.src =
                            (attachment.sizes && attachment.sizes.medium.url) ||
                            attachment.icon;
                        currentPreviewEl = previewDocEl;
                        break;
                    case "zip":
                        if (
                            formatRadios.find(el => el.checked === true).value !==
                            "format_audio-zip" &&
                            formatRadios.find(el => el.checked === true).value !==
                            "format_zip"
                        ) {
    ...
    editAssetButtons.forEach(el =>
        $(el).on("click", function() {
            ...
            if (assetCard.attributes["data-asset-json"].value == "") {
                ...
                var assetData = JSON.parse(
                    assetCard.attributes["data-asset-json"].value
                );
            }
            addAssetDialog.open();
            ...
            addAssetDialogRoot.querySelector(".mdc-dialog__title").innerText =
                "Editing: " + assetData.title;
            addAssetDialogRoot.querySelector("#add-asset__save-button").innerText =
                "Update";

            /* ----------------------- EDIT FORM LABELS AND INPUTS ---------------------- */

            // Asset Title
            addAssetTitle.root_.querySelector(".mdc-floating-label").innerText =
                "Edit Title";
            ...
            artistSelect.foundation_.notchOutline(
                artistSelect.foundation_.adapter_.getLabelWidth()
            );
            ...
            if (assetData.project_id) {
                ...
                projectSelect.foundation_.notchOutline(
                    projectSelect.foundation_.adapter_.getLabelWidth()
                );
            ...
            switch (mimeType.split("/")[0]) {
                case "image":
                    formatRadios.find(el => el.value === "format_image").checked = true;
                    previewThumbEl.querySelector(".preview-file-name").innerHTML = "";
                    previewThumbEl.querySelector(".preview-file-name").style =
                        "display:none";
                    previewThumbEl.classList.remove("icon");
                    currentPreviewEl ? currentPreviewEl.remove() : null;
                    previewImageEl = document.createElement("img");
